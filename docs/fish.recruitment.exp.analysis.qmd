...
2.1 Fit alternative model (no ar term)
...
3.1 Fit model, sampling from data and from priors
```{r recruitment univariate abundance fit brm1a, results = 'markdown', eval = FALSE}
```

```{r recruitment univariate abundance brm1a vars, results = 'markdown', eval = TRUE, error = TRUE, cache = TRUE}

```

The model fit, ggpredict had trouble, and there seems to be an error estimate for every data point (err[1]...err[450]). Compare with `r abnd.brm2.prior %>% get_variables()` (the variables of the model without the ar term)

4.Prior Checks
```{r recruitment univariate abundance brm1a prior checks, results = 'markdown', eval = TRUE}

```
Prior for the intercept and treatment seem appropriate. ar, sd and sderr especially could shrink if further diagnostics reveal problems

4. MCMC diagnostics
```{r recruitment univariate abundance brm1a MCMC diagnostics, results = 'markdown', eval = TRUE}
```
...
the estimate of ar(1) seems a little low compared to the Frequentist analysis
