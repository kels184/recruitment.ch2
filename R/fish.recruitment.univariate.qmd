...
```
In the default control group (BH), and in BQ and DL, the median couldn't be calculated - more than half the values were 0. I will start with normal(0.5, 0.4) as my priors for the intercept 
Since values for this species are low, I might need to be careful about letting the model predict below zero.

For the effects, normal(0,0.4) seems like it should be 

I will keep my other priors from previous models

###Fitting
```{r recruitment univariate sd brmsfit, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate sd brm1 MCMC, results = 'markdown', eval = TRUE}
```
