...
```{r recruitment univariate hm brmsfit NO AC, results = 'markdown', eval = FALSE}
```
...
```{r recruitment univariate hm brm NO AC prior check, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate hm brm NO AC MCMC, results = 'markdown', eval = TRUE, cache = TRUE}
```
All look fine
...
```{r recruitment univariate hm brm NO AC DHARMA, results = 'markdown', eval = TRUE}
...
## Model Investigation
```{r recruitment univariate hm frequentist summary, results = 'markdown', eval = TRUE}
...
```{r recruitment univariate hm brm NO AC summary, results = 'markdown', eval = TRUE}
...
```{r recruitment univariate hm brm NO AC contrasts, results = 'markdown', eval = TRUE}
...
## Summary Figures
```{r recruitment univariate sd figures, results = 'markdown', eval = FALSE}

```

```{r recruitment univariate hmNOAC figures, results="markdown", eval = FALSE}
...
### Temporal autocorrelation
```{r recruitment univariate pt validate autocorrelation, results = 'markdown', eval = TRUE}
```
...
### Temporal autocorrelation
```{r recruitment univariate la validate autocorrelation, results = 'markdown', eval = TRUE}
```
...
## Fit
```{r recruitment univariate sf fit, results = 'markdown', eval = TRUE}
```

treatment model best

## Validate
```{r recruitment univariate sf validate, results = 'markdown', eval = TRUE}
```
fine - no residual problems

```{r  recruitment univariate sf validate autocorrelation, results = 'markdown', eval = TRUE}
```
Cutoff exceeded in DL2, DM4 and W4 (3 times). But 5 patches never saw Siganus fuscescens, so ac unlikely to improve this

```{r recruitment univariate sf refit revalidate, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate sf partial, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate sf priors1, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate sf treatment reorder, results = 'markdown', eval = TRUE}
```


### Fitting
```{r recruitment univariate sf brmsfit, results = 'markdown', eval = FALSE}
```
Divergent transitions? 13 at first. adjusted adapt delta, after which there were 0

```{r recruitment univariate sfNOAC brmsfit, results = 'markdown', eval = FALSE}
```

###Prior Checks
```{r recruitment univariate sf brm1 prior check, results = 'markdown', eval = TRUE}
```
priors good

```{r recruitment univariate sfNOAC brm1 prior check, results = 'markdown', eval = TRUE}
```
Priors good for no ac model

###MCMC

```{r recruitment univariate sf brm1 MCMC, results = 'markdown', eval = TRUE}
```
MCMC checks look good
```{r recruitment univariate sfNOAC brm MCMC, results = 'markdown', eval = TRUE}
```
MCMC look good for no AC model


### DHARMa Residuals
```{r recruitment univariate sf brm1 DHARMa, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate sfNOAC brm DHARMa, results = 'markdown', eval = TRUE}
```

No residual problems

## Model Investigation
```{r recruitment univariate sf frequentist summary, results = 'markdown', eval = TRUE}
```
a low proportion (under 10%) of variance explained. 0.94 ar

```{r recruitment univariate sf brm1 summary, results = 'markdown', eval = TRUE}
```
...
```{r recruitment univariate sf NO AC brm summary, results = "markdown", eval = TRUE}

```
only 7-16% of variance explained

```{r recruitment univariate sf brm contrasts, results = 'markdown', eval = TRUE}
```
```{r recruitment univariate sf NO AC brm contrasts, results = "markdown", eval = TRUE}
...
```{r recruitment univariate sf figures, results = 'markdown', eval = FALSE}
```

```{r recruitment univariate sfNOAC figures, results = 'markdown', eval = FALSE}
...
```{r recruitment univariate size revalidate, results = 'markdown', eval = TRUE}
...
```{r recruitment univariate size frequentist summary, results = 'markdown', eval = TRUE}
...
```{r recruitment univariate size brm contrasts2, results = 'markdown', eval = TRUE}
