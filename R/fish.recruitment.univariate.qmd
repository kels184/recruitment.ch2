...
Definite autocorrelation of the residuals
...
Concerningly, autocorrelation doesn't seem to have been resolved (at lag 1, acf was 0.23, reduced from 0.29)
...
```
autocorrelation the only problem


```{r recruitment univariate hm refit revalidate, results = 'markdown', eval = TRUE}

```
After the refit incorporating autocorrelation, acf is reduced (e.g. at lag 1 and 2: before - 0.15,0.14. after - -.08, .02)
...
Looks like there is autocorrelation, (at least in plotID 1)
```{r recruitment univariate sd refit revalidate, results = 'markdown', eval = TRUE}
```
the ar1 term appears to improve autocorrelation. Possibly a little underdispersed, (but much less than other models I've dealt with)
...
```
NOTE thisis just not the ac model. Very similar to total abundance, but a clear winner with W higher than everything else. BH does appear less than DM
...
```
ac model better by aicc, no residual problems
...
```
Poor fit to the assumed distribution. Heterogeneity of variance
...
Gamma unavailable
Tweedie fits
zigamma for somem reason doesn't fit - this would be my preferred seeing as the brms has hurdle gamma models but not tweedie

```{r recruitment univariate size validate, results = 'markdown', eval = TRUE}

```
tweedie model indicates some problems, however the outlier test indcates too FEW outliers, and the boxplots don't look terrible to me.there are slightly fewer values than expected in the middle area
...
```
Weirdly, there do seem to be differences among treatments - albeit small (<1cm)
