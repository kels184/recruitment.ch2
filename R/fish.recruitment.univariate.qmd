...
```
even though the model was refit with autocorrelation, it still doesn't look like it's being accounted for.
...
```
There were 17 divergent transitions

### Prior checks
```{r recruitment univariate hm brm1 prior check, results = 'markdown', eval = TRUE}
```

priors look quite ok. Maybe ar could come in a bit

### MCMC
```{r recruitment univariate hm brm1 MCMC, results = 'markdown', eval = TRUE, cache = TRUE}
```
sderr slightly autocorrelated at lag 1. One of the posteriors had a low sample size (~0.6). my guess is it will be ar. Or based on the stan_dens plot maybe sderr. density overlay plot a funky shape
...
```
The low ess var was sderr. ar estimate 0.5
