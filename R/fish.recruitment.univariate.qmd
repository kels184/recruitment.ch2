...
```{r chunks, results='markup', eval=TRUE, echo = FALSE, cache = TRUE}
...
```{r loadFunctions, results='markup', eval=FALSE}
```
...
```{r loadPackages, results='markup', eval = TRUE, warnings = FALSE, message = FALSE}
```

- Prepare paths (functions.R)

```{r preparePaths, results='markup', eval = TRUE}

```
- Load Fish Recruitment data (analysis_recruitment.R)
```{r recruitment univariate setup data, results = 'markup', eval = TRUE}
...
## Fit model
```{r recruitment univariate abundance initial models, results = 'markup', eval = TRUE}
```
...
## Validate/Refit
```{r recruitment univariate abundance validate, results = 'markup', eval = TRUE, warning = TRUE}
```
residuals look quite  good.

### Temporal autocorrelation

```{r recruitment univariate abundance validate autocorrelation, results = 'markup', eval = TRUE}
...
```{r recruitment univariate abundance refit autocorrelation, results = 'markup', eval = TRUE}
```
autocorrelation model fit straight away, and is the definite winner by AICc despite higher df

 - Revalidate
```{r recruitment univariate abundance revalidate, results = 'markup', eval = TRUE}
...
## Partial Plot
```{r recruitment univariate abundance partial, results='markup', eval = TRUE}
...
## Bayesian Model
### 1 Initial priors
```{r recruitment univariate abundance priors1, results = 'markup', eval = TRUE}
...
```{r recruitment univariate abundance priors2, results = 'markup', eval = TRUE}
...
```{r recruitment univariate abundance vars, results = 'markup', eval = TRUE, error = TRUE, cache = TRUE}
...
```{r recruitment univariate abundance brm1b prior checks, results = 'markup', eval = TRUE, cache = TRUE}
...
### 4 MCMC diagnostics
```{r recruitment univariate abundance brm1b MCMC diagnostics, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate abundance DHARMA residuals, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate abundance brm ac, results = "markup", eval = TRUE}
...
```{r recruitment univariate abundance summary, results = 'markup', eval = TRUE, error =TRUE}
...
### 2 Bayesian Model
 - 2.1 Summary
```{r recruitment univariate abundance brm summary, results = 'markup', eval = TRUE, error =TRUE}
...
the estimate of ar(1) is slightly lower compared to the Frequentist analysis (0.8 vs 0.9)
 - 2.2 R-squared
```{r recruitment univariate abundance brm r square, results = 'markup', eval = TRUE}
...
```{r recruitment univariate abundance all contrasts, results = 'markup', eval = TRUE, warning=TRUE}
...
```{r recruitment univariate abundance planned contrasts, results = 'markup', eval = TRUE, warning=TRUE}
...
## Summary figures
```{r recruitment univariate abundance summary figure, results = 'markup', eval = TRUE}
...
## Fit model
```{r recruitment univariate sp fit, results = 'markup', eval = TRUE}
```
As with abundance, the Treatment + RE of plot model performed best, despite consuming more df

##Validate/Refit
```{r recruitment univariate sp validate, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp validate autocorrelation, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp refit autocor, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp revalidate, results = 'markup', eval = TRUE, cache = TRUE}

```
Similar problems evident (underdispersion), but without the red flag for the Levene test. 

```{r recruitment univariate sp revalidate autocor, results = 'markup', eval = TRUE, cache = TRUE}
...
## Partial Plot
```{r recruitment univariatesp partial, results='markup', eval = TRUE}
 
```

## Bayesian Model
### 1 Initial priors
```{r recruitment univariate sp priors1, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp priors2, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp prior fit, results = 'markup', eval = FALSE}
```

```{r recruitment univariate sp brm1.prior check, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sp brmsfit, results = 'markup', eval = FALSE}
```
...
### 3 Prior Checks
```{r recruitment univariate sp brm1a prior checks, results = 'markup', eval = TRUE, cache = TRUE}
...
```{r recruitment univariate sp MCMC1, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp MCMC2, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp brms refit, results = 'markup', eval = FALSE}

```
2121 divergent transitions

```{r recruitment univariate sp recheck priors and mcmc, results = 'markup', eval = TRUE, cache = TRUE}

```
ess and ac now acceptable

```{r recruitment univariate sp recheck recheck mcmc2, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp refit again, results = 'markup', eval = FALSE}
```
This model had no divergent transitions

```{r recruitment univariate sp priors mcmc sp.brm1f, results = 'markup', eval = TRUE, cache = TRUE}
...
```{r recruitment univariate sp DHARMA residuals, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sp frequentist summary, results = 'markup', eval = TRUE, error = TRUE}
```
...
### 2 Bayesian
```{r recruitment univariate sp bayesian summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sp all contrasts, results = 'markup', eval = TRUE }

```
 - Renderuit at this point due to 'sp.end.brm1f' not found Similarly to the abundance model, there is high (>95%) confidence in differences in all contrasts EXCEPT BQ-DL

# HALICHOERES MINIATUS Abundance
## Fit
```{r recruitment univariate hm fit, results = 'markup', eval = TRUE}

```
The best model for halichoeres miniatus was the Treatment model (with random intercept)
## Validate
```{r recruitment univariate hm validate, results = 'markup', eval = TRUE}

```

```{r recruitment univariate hm validate autocor, results = 'markup', eval = TRUE}

```
Autocorrelation not of too much concern - the cutoff was exceeded only twice(in BH5)


```{r recruitment univariate hm refit revalidate, results = 'markup', eval = TRUE}
...
```{r recruitment univariate hm partial, results = 'markup', eval = TRUE}
...
###Initial priors
```{r recruitment univariate hm priors1, results = 'markup', eval = TRUE}
...
### Fitting
```{r recruitment univariate hm brmsfit, results = 'markup', eval = FALSE}
```
...
```{r recruitment univariate hm brmsfit NO AC, results = 'markup', eval = FALSE}
```
No divergent transitions

### Prior checks
```{r recruitment univariate hm brm1 prior check, results = 'markup', eval = TRUE}
```

priors look quite ok

```{r recruitment univariate hm brm NO AC prior check, results = 'markup', eval = TRUE}
```

### MCMC
```{r recruitment univariate hm brm1 MCMC, results = 'markup', eval = TRUE, cache = TRUE}
```
...
```{r recruitment univariate hm brm NO AC MCMC, results = 'markup', eval = TRUE, cache = TRUE}
```
All look fine


### DHARMA residuals
```{r recruitment univariate hm brm1 DHARMA, results = 'markup', eval = TRUE}

```

Something is a bit strange here - different number of groups to Treatment. Underdispersion

```{r recruitment univariate hm brm NO AC DHARMA, results = 'markup', eval = TRUE}
...
## Model Investigation
```{r recruitment univariate hm frequentist summary, results = 'markup', eval = TRUE}

```

```{r recruitment univariate hm brm1 summary, results = 'markup', eval = TRUE}
...
```{r recruitment univariate hm brm NO AC summary, results = 'markup', eval = TRUE}

```
Some evidence contrasts (BH -BQ, BH-DL) (same story as frequentist)
Low proportion of variance explained (11.6 - 15.8% with the random intercept)


```{r recruitment univariate hm brm contrasts, results = 'markup', eval = TRUE}

```

```{r recruitment univariate hm brm NO AC contrasts, results = 'markup', eval = TRUE}

```
   contrast    `P>`    `P<` evidence   
   <chr>      <dbl>   <dbl> <chr>      
 1 BH - BQ  1       0       very strong
 2 BH - DL  0.992   0.00833 very strong
 3 BH - DM  0.797   0.203   no evidence
 4 BH - W   0.49    0.51    no evidence
 5 BQ - DL  0.0562  0.944   evidence   
 6 BQ - DM  0.00208 0.998   very strong
 7 BQ - W   0       1       very strong
 8 DL - DM  0.0446  0.955   strong     
 9 DL - W   0.00958 0.990   very strong
10 DM - W   0.201   0.799   no evidence

## Summary Figures
```{r recruitment univariate hm figures, results = 'markup', eval = TRUE}

```

# SIGANUS DOLIATUS Abundance
## Fit
```{r recruitment univariate sd fit, results = 'markup', eval = TRUE}
...
## Validate
```{r recruitment univariate sd validate, results = 'markup', eval = TRUE}
...
```{r recruitment univariate sd validate autocorrelation, results = 'markup', eval = TRUE}

```
There is autocorrelation, the vutoff was exceeded 7 times, at least once in all treatments

```{r recruitment univariate sd refit revalidate, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sd partial, results = 'markup', eval = TRUE}
```
...
## Bayesian Model
### Initial priors
```{r recruitment univariate sd priors1, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sd treatment reorder, results = 'markup', eval = TRUE}
```

###Fitting
```{r recruitment univariate sd brmsfit, results = 'markup', eval = FALSE}
```
no problem re divergent transitions, that's nice

###Prior Checks
```{r recruitment univariate sd brm2 prior check, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sd brm2 MCMC, results = 'markup', eval = TRUE}
```
...
### DHARMa Residuals
```{r recruitment univariate sd brm2 DHARMa, results = 'markup', eval = TRUE}
```
...
## Model Investigation
```{r recruitment univariate sd frequentist summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sd brm2 summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sd brm contrasts, results = 'markup', eval = TRUE}
```
...
## Summary Figures
```{r recruitment univariate sd figures, results = 'markup', eval = FALSE}

```

```{r recruitment univariate hmNOAC figures, results="markup", eval = FALSE}
...
## Fit
```{r recruitment univariate ps fit, results = 'markup', eval = TRUE}
...
## Validate
```{r recruitment univariate ps validate, results = 'markup', eval = TRUE}

```
no residual problems, 

### Temporal autocorrelation
```{r recruitment univariate ps validate autocorrelation, results = 'markup', eval = TRUE}
```

Looks like there is autocorrelation, cutoff exceeded 5 times (all treatments)

```{r recruitment univariate ps refit revalidate, results = 'markup', eval = TRUE}
```
ac model better by aicc, no residual problems. Autocorrelation slightly reduced, 3 instead of 5 times cutoff exceeded

## Partial Plot

```{r recruitment univariate ps partial, results = 'markup', eval = TRUE}
```
...
## Bayesian Model
### Initial priors
```{r recruitment univariate ps priors1, results = 'markup', eval = TRUE}
```
initial priors will be 0, 0.4 for the intercept, 0,2 for the effects

```{r recruitment univariate ps treatment reorder, results = 'markup', eval = TRUE}
```

###Fitting
```{r recruitment univariate ps brmsfit, results = 'markup', eval = FALSE}
```
no divergent transitions

###Prior Checks
```{r recruitment univariate ps brm2 prior check, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate ps brm2 MCMC, results = 'markup', eval = TRUE}
```
MCMC checks look good

### DHARMa Residuals
```{r recruitment univariate ps brm2 DHARMa, results = 'markup', eval = TRUE}
```
...
## Model Investigation
```{r recruitment univariate ps frequentist summary, results = 'markup', eval = TRUE}
```
R -squared 43 - 57% explained with (frequentist). ar ~0.81

```{r recruitment univariate ps brm2 summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate ps brm contrasts, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate ps figures, results = 'markup', eval = TRUE}
```

# POMACENTRUS TRIPUNCTATUS Abundance
## Fit
```{r recruitment univariate pt fit, results = 'markup', eval = TRUE}
```

treatment model best

## Validate
```{r recruitment univariate pt validate, results = 'markup', eval = TRUE}
```
some problems already - slight non-uniformity, slight bit of heterogeneity. I think it's ok. We'll see with the ac model 


### Temporal autocorrelation
```{r recruitment univariate pt validate autocorrelation, results = 'markup', eval = TRUE}
```

there is evidence of autocorrelation - cutoff exceeded 10 times

```{r recruitment univariate pt refit revalidate, results = 'markup', eval = TRUE}
```
ac model better by aicc, seems, has improved autocorrelation (3 times exceeded, 2 of those times it was in patches where Pomacentrus tripunctatus was never recorded, BQ4 and DL1), uniformity now ok. a little underdispersed (more conservative). I'm happy with the box plots despite the warning

## Partial Plot

```{r recruitment univariate pt partial, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate pt priors1, results = 'markup', eval = TRUE}
```

once again I need to reorder. I'll start with normal(0, 1) for the intercept, and normal(0,3) for the effects

```{r recruitment univariate pt treatment reorder, results = 'markup', eval = TRUE}
```


### Fitting
```{r recruitment univariate pt brmsfit, results = 'markup', eval = FALSE}
```
no divergent transitions

###Prior Checks
```{r recruitment univariate pt brm1 prior check, results = 'markup', eval = TRUE}
```
Look ok to me. ar could maybe be positively restricted

###MCMC

```{r recruitment univariate pt brm1 MCMC, results = 'markup', eval = TRUE}
```
MCMC checks look good

### DHARMa Residuals
```{r recruitment univariate pt brm1 DHARMa, results = 'markup', eval = TRUE}
```
once again, apparently underdispersed

## Model Investigation
```{r recruitment univariate pt frequentist summary, results = 'markup', eval = TRUE}
```
R -squared 43 - 57% explained with (frequentist). ar ~0.98

```{r recruitment univariate pt brm1 summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate pt brm contrasts, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate pt figures, results = 'markup', eval = TRUE}
```
...
## Fit
```{r recruitment univariate la fit, results = 'markup', eval = TRUE}
```

treatment model better than the plot.weight model, but not really any better than the null (random intercept only) model

## Validate
```{r recruitment univariate la validate, results = 'markup', eval = TRUE}
```
fine

### Temporal autocorrelation
```{r recruitment univariate la validate autocorrelation, results = 'markup', eval = TRUE}
```
Some autocorrelation - exceeded 3 times (BH5, DM1, W4). Also 2 patches where Lethrinus atkinsoni never occurred, so the ac model may only be able to reduce this to 2 if the same thing happens as for P tripunctatus

```{r recruitment univariate la refit revalidate, results = 'markup', eval = TRUE}
```
ac model better by aicc, now better than the null. seems to have improved autocorrelation - 2 times exceeded (DL2 and BQ2, the patches in which this species was never recorded). a little underdispersed (more conservative)

Based on the aicc and the slight improvement to autocorrelation, I choose this model

## Partial Plot

```{r recruitment univariate la partial, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate la priors1, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate la treatment reorder, results = 'markup', eval = TRUE}
```


### Fitting
```{r recruitment univariate la brmsfit, results = 'markup', eval = FALSE}
```
1 divergent. I'm ok with that

###Prior Checks
```{r recruitment univariate la brm1 prior check, results = 'markup', eval = TRUE}
```
priors good
...
```{r recruitment univariate la brm1 MCMC, results = 'markup', eval = TRUE}
```
MCMC checks look good

### DHARMa Residuals
```{r recruitment univariate la brm1 DHARMa, results = 'markup', eval = TRUE}
```
again, a weird number of catPreds - this time only 3! I;m going to ignore that side. No apparent under or overdispersion

## Model Investigation
```{r recruitment univariate la frequentist summary, results = 'markup', eval = TRUE}
```
a low proportion (under 10%) of variance explained. 0.79 ar

```{r recruitment univariate la brm1 summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate la brm contrasts, results = 'markup', eval = TRUE}
```
BQ-DM, and W vs BH, BQ and DL the contrasts with evidence

## Summary Figures

```{r recruitment univariate la figures, results = 'markup', eval = TRUE}
```
...
## Fit
```{r recruitment univariate sf fit, results = 'markup', eval = TRUE}
```

treatment model best

## Validate
```{r recruitment univariate sf validate, results = 'markup', eval = TRUE}
```
fine - no residual problems

```{r  recruitment univariate sf validate autocorrelation, results = 'markup', eval = TRUE}
```
Cutoff exceeded in DL2, DM4 and W4 (3 times). But 5 patches never saw Siganus fuscescens, so ac unlikely to improve this

```{r recruitment univariate sf refit revalidate, results = 'markup', eval = TRUE}
```
ac slightly worse aicc, now better than the null.  a little underdispersed (more conservative). seems to have WORSENED autocorrelation - cutoff exceeded 6 times - everywhere this species wasn't seen, plus DL2 (lag 4)

Based on worse AIC, underdispersion and worsened autocorrelation I think I need to use a non AC model for Siganus fuscescens

## Partial Plot

```{r recruitment univariate sf partial, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sf priors1, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sf treatment reorder, results = 'markup', eval = TRUE}
```


### Fitting
```{r recruitment univariate sf brmsfit, results = 'markup', eval = FALSE}
```
Divergent transitions? 13 at first. adjusted adapt delta, after which there were 0

```{r recruitment univariate sfNOAC brmsfit, results = 'markup', eval = FALSE}
```

###Prior Checks
```{r recruitment univariate sf brm1 prior check, results = 'markup', eval = TRUE}
```
priors good

```{r recruitment univariate sfNOAC brm1 prior check, results = 'markup', eval = TRUE}
```
Priors good for no ac model

###MCMC

```{r recruitment univariate sf brm1 MCMC, results = 'markup', eval = TRUE}
```
MCMC checks look good
```{r recruitment univariate sfNOAC brm MCMC, results = 'markup', eval = TRUE}
```
MCMC look good for no AC model


### DHARMa Residuals
```{r recruitment univariate sf brm1 DHARMa, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sfNOAC brm DHARMa, results = 'markup', eval = TRUE}
```

No residual problems

## Model Investigation
```{r recruitment univariate sf frequentist summary, results = 'markup', eval = TRUE}
```
a low proportion (under 10%) of variance explained. 0.94 ar

```{r recruitment univariate sf brm1 summary, results = 'markup', eval = TRUE}
```
...
```{r recruitment univariate sf NO AC brm summary, results = "markup", eval = TRUE}

```
only 7-16% of variance explained

```{r recruitment univariate sf brm contrasts, results = 'markup', eval = TRUE}
```
```{r recruitment univariate sf NO AC brm contrasts, results = "markup", eval = TRUE}

```


 1 BH - BQ  0.638  0.362  no evidence
 2 BH - DL  0.42   0.58   no evidence
 3 BH - DM  0.0233 0.977  strong     
 4 BQ - DL  0.289  0.711  no evidence
 5 BQ - DM  0.0112 0.989  strong     
 6 DL - DM  0.0321 0.968  strong     
 7 W - BH   0.96   0.04   strong     
 8 W - BQ   0.981  0.0192 strong     
 9 W - DL   0.938  0.0617 evidence   
10 W - DM   0.35   0.65   no evidence
...
```{r recruitment univariate sf figures, results = 'markup', eval = FALSE}
```

```{r recruitment univariate sfNOAC figures, results = 'markup', eval = FALSE}
...
```{r recruitment univariate size data, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size fit, results = 'markup', eval = TRUE}

```
The base (random intercept only) performed the best. However we'll keep looking at the Treatment model for now

## Validate 
```{r recruitment univariate size validate, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size refit, results = 'markup', eval = TRUE, error = TRUE}
...
```{r recruitment univariate size revalidate, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size validate autocorrelation, results = 'markup', eval = TRUE}

```
Autocorrelation in the tweedie model is low and I won't worry about fitting an ac model.

##Partial Plot
```{r recruitment univariate size partial, results = 'markup', eval = TRUE}

```

##Bayesian Model
### Priors
```{r recruitment univariate size priors, results = 'markup', eval = TRUE}

```


### Fitting
```{r recruitment univariate size brmsfit, results = 'markup', eval = FALSE}
...
```{r recruitment univariate size brm1 prior checks, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size brm2 fit, results = 'markup', eval = FALSE}

```
refit in 8 mins

```{r recruitment univariate size brm2 prior checks, results = 'markup', eval = TRUE}
```
Prior distributions look fine, wide in most cases but not shifting any posteriors

## MCMC

```{r recruitment univariate sf brm2 MCMC, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size brm DHARMa, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size frequentist summary, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size brm summary, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size brm contrasts, results = 'markup', eval = TRUE}
...
```{r recruitment univariate size brm contrasts2, results = 'markup', eval = TRUE}
